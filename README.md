...
**Built with AI collaboration: Claude (Anthropic) — lead, alongside Google Gemini, ChatGPT (OpenAI), and GitHub Copilot. See Acknowledgements below.**
...
- Deep sleep between LoRa sniff windows; a Channel Activity Detection (CAD) scan checks
  for LoRa preamble energy on-air before the radio commits to a full long-preamble
  Wake-on-Radio (WOR) receive window triggered from the hub — see **Power Optimization** below
...
**Wake sequence: CAD + WOR (5000-symbol) + Deep Sleep.** Rather than parking directly in a
long-preamble WOR receive window, the outside node first runs a low-power SX1262 Channel
Activity Detection (CAD) scan. CAD checks briefly for LoRa preamble energy on-air; only if
activity is detected does the radio commit to the full 5000-symbol WOR receive window to
demodulate the incoming trigger from the hub. Between checks, the node returns to deep sleep.
This keeps the "am I being paged?" check cheaper than sitting in a full receive window on
every sniff cycle.
...
**Measured results (PPK2, CAD-based, current):**

| Metric                                  | Value             |
|------------------------------------------|--------------------|
| Deep sleep floor                         | ~21.93 µA          |
| Active spike (avg / max)                 | ~7.43 mA / ~14.23 mA |
| Full duty-cycle average (active + sleep) | **~38.19 µA**      |
| WOR wake event (avg / max)               | ~40.64 mA / ~105.44 mA |
| Theoretical battery life (3000 mAh LiPo) | ~107.7 months      |
| Realistic (derated) battery life         | ~62–90 months      |

**Methodology note — CAD supersedes autoDutyCycle:** The Hackster write-up's published figures
(51.30 µA average, ~83.5 months) were measured under the earlier `autoDutyCycle` WOR detection
method. The project has since moved to **CAD (Channel Activity Detection)** as the pre-check
ahead of the WOR receive window, which measurably reduces average current — **38.19 µA vs.
51.30 µA (a 25.6% reduction)**, improving theoretical battery life by **34.3%** (~107.7 months
vs. ~80.2 months) on the same 3000 mAh LiPo. The 51.30 µA / ~83.5-month figures should be
considered superseded, not a number to reconcile against.

Full PPK2 capture screenshots and raw observations (CAD + WOR + Deep Sleep):
[Nordic PPK2 Power Observations — Outside Node](https://gist.github.com/Tech500/26b9f16bd595f98c8c41618e758c92f0)
...
| Wake pre-check            | CAD (Channel Activity Detection)     |
| Sync word                | Private                               |
...
- Arduino ESP32 Core 3.3.10   --No exceptions --code required for ESP-NOW and perherial shutdowns. 
